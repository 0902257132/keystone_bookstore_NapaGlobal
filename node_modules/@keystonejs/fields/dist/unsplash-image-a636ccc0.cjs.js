...
const Search = ({
  ...
  const [getUnsplashImages, {
    data: {
      searchUnsplash: searchResults
    } = {}
  }] = reactHooks.useLazyQuery(UNSPLASH_SEARCH_QUERY, {
    variables: {
      query: searchTerm,
      page: searchPage,
  ...
  const showNext = React.useCallback(() => {
    setSearchPage(Math.min(searchResults && searchResults.totalPages || Infinity, searchPage + 1));
  }, [searchPage]);
...
function serialize({
  ...
  return {
    mutations,
    node: _objectSpread(_objectSpread({}, node.toJSON()), {}, {
